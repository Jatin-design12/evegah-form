...
app.get("/api/returns", async (_req, res) => {
  try {
    const { rows } = await pool.query(
      `select * from public.returns order by created_at desc`
    );
    res.json(rows || []);
  } catch (error) {
    res.status(500).json({ error: String(error?.message || error) });
...
app.post("/api/returns/submit", upload.array("photos", 10), async (req, res) => {
  ...
  try {
    ...
    const rentalQ = await client.query(`select id, rider_id from public.rentals where id = $1`, [
      rentalId,
    ]);
...
app.get("/api/rentals/active", async (req, res) => {
  ...
  try {
    const params = [];
    const where = [
      `not exists (select 1 from public.returns ret where ret.rental_id = public.rentals.id)`,
    ];
    const push = (v) => {
      params.push(v);
      return `$${params.length}`;
    ...
    const { rows } = await pool.query(
      `select * from public.rentals where ${where.join(" and ")} order by start_time desc limit 1`,
      params
    );
